...
public class SafeProperties extends Properties {
	...
	//只是用于配置下发上
	public synchronized void load1(InputStream inStream) throws IOException {
		...
		while (true) {
			...
			if (line.length() > 0) {
				...
				if (!line.trim().startsWith("# ") && line.contains("=") && !line.contains("\"") && (firstChar != '!')) {
					while (continueLine(line)) {
						String nextLine = in.readLine();
						intactLine = intactLine + "\n" + nextLine;
						if (nextLine == null)
	...
	public synchronized Object put(Object key, Object value, String line,int i) {
		context.putOrUpdate(key.toString(), value.toString(), line, i);
		return super.put(key, value);
	}
	...
	class PropertiesContext {
		...
		public void putOrUpdate(String key, String value, String line,int i) {
			PropertyEntry pe = new PropertyEntry(key, value, line, i);
			removesame(key,line,i);
			commentOrEntrys.add(pe);
		}
		...
		public int removesame(String key,String line, int i) {
			for (int index = 0; index < commentOrEntrys.size(); index++) {
				Object obj = commentOrEntrys.get(index);
				if (obj instanceof PropertyEntry) {
					if (obj != null) {//必须是某一行，某一行的值均相等才可移除
						if (key.equals(((PropertyEntry) obj).getKey()) && line.equals(((PropertyEntry) obj).getLine()) && i == ((PropertyEntry) obj).getInline()) {
		...
		class PropertyEntry {
			 ...
			 * @param line
			 */
			public PropertyEntry(String key, String value, String line,int i) {
				this(key, value);
				this.line = line;
				this.inline = i;
			}
