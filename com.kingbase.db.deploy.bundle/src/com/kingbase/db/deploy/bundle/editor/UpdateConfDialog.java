...
public class UpdateConfDialog extends Dialog {
	...
	public UpdateConfDialog(Shell shell, List<KeyValueEntity> listkv, Map<String, Session> sessionMap, CNodeEntity entity, String type, CreateMasterStatusEditor editor) {
		super(shell);
		this.shell = shell;
		this.listkv = listkv;
	...
	private void createDialog() {
		...
		btnApply.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				StringBuffer buf = new StringBuffer("sed -i");
				int i = 0;
				for (KeyValueEntity cell : listkv) {
					if (!cell.getValue().equals(cell.getOldValue())) {
						if(cell.getKey().startsWith("#")){//注释的参数若是修改了，则相当于改为开启
							String key = cell.getKey().substring(1, cell.getKey().length());
							buf.append(" -e \"s@"+cell.getKey()+"="+cell.getOldValue()+"@"+key+"="+cell.getValue().trim()+"@\"");//@用作分隔符
						}else{
							buf.append(" -e \"s@"+cell.getKey()+"="+cell.getOldValue()+"@"+cell.getKey()+"="+cell.getValue().trim()+"@\"");//@用作分隔符
						}
						i++;
					}
				}
				...
				if (btnAll.getSelection()) {
						...
						if (type.equals("kingbasecluster.conf")) {
							...
							for (TableNodeEntity nodepool : listpool) {
								...
								if (str.equals(key)) {
									...
									try {
										JschUtil.execCommand1(session, buf.toString() + " " + nodepool.getNodeEntity().getdPath() + "kingbasecluster/etc/kingbasecluster.conf");
									} catch (Exception e1) {
										e1.printStackTrace();
									}
									break;
								...
								if (str.equals(key)) {
									...
									try {
										if (type.equals("kingbase.conf")) {
											JschUtil.execCommand1(session, buf.toString() + " " + nodeDB.getNodeEntity().getdPath() + "db/data/kingbase.conf");
											JschUtil.execCommand1(session, buf.toString() + " " + nodeDB.getNodeEntity().getdPath() + "db/etc/kingbase.conf");
										}
									} catch (Exception e1) {
										e1.printStackTrace();
									}
					...
					for (String key : sessionMap.keySet()) {
						...
						if (str.equals(key)) {
							...
							try {
								if (type.equals("kingbase.conf")) {
									JschUtil.execCommand1(session, buf.toString() + " " + entity.getdPath() + "db/data/kingbase.conf");
									JschUtil.execCommand1(session, buf.toString() + " " + entity.getdPath() + "db/etc/kingbase.conf");
								} else if (type.equals("kingbasecluster.conf")) {
									JschUtil.execCommand1(session, buf.toString() + " " + entity.getdPath() + "kingbasecluster/etc/kingbasecluster.conf");
								}
							} catch (Exception e1) {
								e1.printStackTrace();
							}
