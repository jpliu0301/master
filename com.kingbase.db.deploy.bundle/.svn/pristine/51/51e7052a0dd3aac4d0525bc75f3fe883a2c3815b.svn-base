...
//		dataGroup.heightHint = 350;
		...
		if (oldReadEntity != null) {
			...
			String str1 = oldReadEntity.getPoolEntity().getPoolzipPath();
...
//				txt6.setText(oldReadEntity.getPoolEntity().getNetcard());
				...
				String str11 = oldReadEntity.getPoolEntity().getPoolPath();
				if(str11!=null&&!str11.trim().equals("")){
					
					try {
						str11 = java.net.URLDecoder.decode(str11, "utf-8");
					} catch (UnsupportedEncodingException e1) {
						e1.printStackTrace();
					}
					txtpost.setText(str11);
				}
